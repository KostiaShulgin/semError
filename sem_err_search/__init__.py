...
def __check_semantic_compatibility(step_id):
    ...
    if len(data_instances) != 0:
        ...
        if len(element_names) == 0:
            ...
            if formatted_class_name not in [class_name.name.split('.')[0] for class_name in __ProOnto.classes()]:
...
def process(scenario_path, pro_path):
    global __ProOnto
    global __ScenOnto
    __ProOnto = get_ontology(pro_path).load()
    ...
    if len(incompatibilities) == 0:
        ...
        for incompatibility in incompatibilities:
            print('Semantic incompatibility found!')
            print('Data type \"' + incompatibility[0] + '\" used in \"' + incompatibility[
                1] + '\" element is not found in ontology.')
            print('Please make sure that You\'re using right terminology\n')
...
def __process_steps(steps_ids, visited_ids):
    ...
    for step_id in steps_ids:
        ...
        if [idRel[0] for idRel in __ScenOnto.id.get_relations() if idRel[1] == step_id][0] in __ScenOnto.get_instances_of(
                __ScenOnto.endEvent):
            return
        else:
...
def get_start_id():
    ...
    not_source_names = [idRel[0] for idRel in __ScenOnto.id.get_relations() if idRel[1] not in
                        [sourceRel[1] for sourceRel in __ScenOnto.targetRef.get_relations()]]
